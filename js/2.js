window.onload=function(){
	...
	function move(){
			...
			for(var i=0;i<imgs.length;i++){
				...
				animate(bj[i],{opacity:0},200)
				lis[i].style.background='#211616';
			}
			animate(imgs[n],{opacity:1},200)
			animate(bj[n],{opacity:1},200,function(){
				ssflag=true;
			})
	...
	for(var i=0;i<lis.length;i++){
			...
			lis[i].onclick=function(){
				...
				animate(bj[this.index],{opacity:1},200,function(){
					ssflag=true;
				})
				n=this.index;
		}
